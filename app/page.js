...
export default function Home() {
  ...
  if (!isLoading && (activites)) {
    var activitesToDisplay = null;
    if (userRole == Roles.Manager || userRole == Roles.CA) 
      activitesToDisplay = activites;
    else if(userRole == Roles.CSI)
      activitesToDisplay = activites.filter((act) => {
        return act.id_ct == userid;
      });
    else if(userRole == Roles.Intervenant)
      activitesToDisplay = activites.filter((act) => {
        return act.id_exec == userid;
      });

    activitesDisplay = <div className='flex'>{activitesToDisplay.map((activity, key) => (<div key={key}><div className='w-48 max-w-xs'><ActiviteComponent activite={activity} /></div></div>))}</div>;
  }
